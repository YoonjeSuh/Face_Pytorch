...
from dataset.mycasia import CASIAWebFace
from dataset.mylfw import LFW
...
def train(args):
    ...
    datasetsize=len(trainset.image_list)
    ...
    if args.backbone == 'MobileFace':
        ...
        net = CBAMResNet(100, feature_dim=args.feature_dim, mode='ir') #default dim 500,batch 200
    ...
    '''
    criterion = CenterLoss(trainset.class_nums, args.feature_dim).to(device)
    optimizer_ft=optim.SGD(criterion_center.parameters(), lr=0.5)
    exp_lr_scheduler = lr_scheduler.MultiStepLR(optimizer_ft, milestones=[6, 11, 16], gamma=0.1)
    print("check1")
    ...
    best_cfp_fp_iters = 0
    total_iters = 0
    vis = Visualizer(env=args.model_pre + args.backbone) #Error! -> connect visdom server. python -m visdom.server
    print("check3")
    for epoch in range(1, args.total_epoch + 1):
        ...
        for data in trainloader:
            ...
            print("{:.2f}".format(count*args.batch_size*100/datasetsize)) #shows progress in percentage
            #print(img.shape)
            # Train accuracy currently zero???
            raw_logits = net(img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_root', type=str, default='/home/jimmyok/project/new_casia', help='train image root')
    parser.add_argument('--train_file_list', type=str, default='/home/jimmyok/project/new_casia/xaa', help='train list') #split files
    parser.add_argument('--lfw_test_root', type=str, default='/home/jimmyok/project/new_lfw', help='lfw image root')
    parser.add_argument('--lfw_file_list', type=str, default='/home/jimmyok/project/new_lfw/pairs.txt', help='lfw pair file list')
    '''
